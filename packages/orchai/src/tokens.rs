...
pub type TokenHumanOptional = (String, Option<Uint256>);

pub type Tokens = Vec<Token>;
pub type TokensHuman = Vec<TokenHuman>;

pub type TokensHumanOptional = Vec<TokenHumanOptional>;
...
impl TokensToRaw for TokensHuman {
    fn to_raw(&self, deps: Deps) -> StdResult<Tokens> {
        let collaterals: Tokens = self
            .iter()
            .map(|c| Ok((deps.api.addr_canonicalize(&(c.0.to_string()))?, c.1)))
